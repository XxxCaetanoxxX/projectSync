...
import { EmailProducerService } from 'src/email_queue/email.producer.service';
...
export class TicketService {
  ...
  async buyTicket(ticketTypeId: number, userId: number) {
    const ticketData = await this.prisma.withAudit.$transaction(async (tx) => {
      ...
      await this.emailqueue.sendTicketBoughtEmail({ username: ticket.user.name, ticketName, eventName: ticketType.event.name, email: ticket.user.email, ticketId: ticket.id });
